...
code = code.replace(
  ...
  'className="bg-[#0B1D3A] rounded-2xl p-6 md:p-8 shadow-xl mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative overflow-hidden"'
);
...
code = code.replace(
  ...
  'bg-emerald-500/20 text-emerald-400 text-[10px] font-bold px-2 py-1 rounded uppercase tracking-widest border border-emerald-500/30 backdrop-blur-sm'
);
...
code = code.replace(
  ...
  'bg-blue-500/20 text-blue-300 text-[10px] font-bold px-2 py-1 rounded uppercase tracking-widest border border-blue-500/30 backdrop-blur-sm'
);
...
code = code.replace(
  ...
  'className="block w-full pl-10 pr-3 py-3 border border-blue-400/20 rounded-xl leading-5 bg-[#142A4A]/80 text-white placeholder-blue-300/50 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500 transition-all sm:text-sm backdrop-blur-md"'
);
...
// We need to fix the JSX comment syntax
code = code.replace('<!-- Decoration -->', '{/* Decoration */}');
